...
if __name__ == "__main__":
    ...
    print(f"Running tests for {__file__} this will take a few seconds.")
    ...
    with test_timer:
        ...
        t.stop()
        print(f"Basic usage: {t.duration_as_delta}~={TIMER_STATIC_DURATION}")

        with Timer() as t:
            sleep(TIMER_STATIC_DURATION)
        print(f"Context Manager usage: {t.duration_as_delta}~={TIMER_STATIC_DURATION}")
        ...
        SLICES = 10
        t.start()
        for time_snippet in range(SLICES):
            sleep(TIMER_STATIC_DURATION / SLICES)
            t.lap()
        t.stop()
        ...
        for c, lap in enumerate(t.get_laps()):
            ...
            print(
                "lap #{c+1}\t lap_start: {lap_start}"
                "\tlap_end\t{lap_end}, "
                "lap_duration\t{lap_end-lap_start}~=({TIMER_STATIC_DURATION / slices})"
            )

    print(f"Tests completed in {test_timer.duration_as_delta}")
